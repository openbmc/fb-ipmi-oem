...
std::string getMotherBoardFruPath()
{
    ...
    auto predNth = [targetIndex, counter = size_t{0}]
...
std::vector<std::pair<std::string, std::string>> getHostFWVersions(int hostNum)
{
    ...
    {
        auto objPath = "/xyz/openbmc_project/software/host" +
                       std::to_string(hostNum) + "/" + fwType;
        auto version = getFWVersion(objPath);
        result.emplace_back(fwType, version);
    }
    return result;
}
