                         ...
                         ipmi_context_t)
{
    ...
    *data_len = 1;
    ...
    std::map<sdbusplus::message::object_path,
             std::map<std::string, std::map<std::string, PropertyValue>>> objects;
    ...
    for (const auto& [path, interfaces] : objects) {
        if (path.str.find(rootPath) != 0) continue;
            ...
            if (busIt != properties.end() && addrIt != properties.end()) {
                try {
                    ...
                    if (busValue == targetBus && addrValue == targetAddress) {
                        std::cout << "Match found at index: " << index << std::endl;
                    ...
                    std::cerr << "️Unexpected property type at index " << index << std::endl;
                    return -1;
                }
            }
			index++;
