...
void sysConfig(std::vector<std::string>& data, size_t pos)
{
    ...
    try
    {
        ...
        {
            std::string speedSize = oemData[dimmInfo][info][DIMM_SPEED];
            ...
            std::string type = oemData[dimmInfo][info][DIMM_TYPE];
            std::string dualInlineMem = oemData[dimmInfo][info][KEY_DIMM_TYPE];
            strToBytes(type, res);
    ...
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "DIMM information not available\n");
    }
}
...
void procInfo(std::string& result, size_t pos)
{
    ...
    try
    {
        std::string procName = oemData[procInfo][procIndex][KEY_PROC_NAME];
        std::string basicInfo = oemData[procInfo][procIndex][KEY_BASIC_INFO];
        ...
        auto productName = ipmi::boot::instances(s);
...
ipmi::RspType<> ipmiOemQSetProcInfo(ipmi::Context::ptr ctx,
                                    std::vector<uint8_t> reqData)
{
    qProcInfo_t* req = reinterpret_cast<qProcInfo_t*>(reqData.data());
    ...
    oemData[procInfo][ss.str()][KEY_PROC_INDEX] = req->procIndex;
    str = bytesToStr(reqData.data() + 5, len);
    oemData[procInfo][ss.str()][cpuInfoKey[req->paramSel]] = str.c_str();
    flushOemData();
    return ipmi::responseSuccess();
}
...
//
ipmi::RspType<std::vector<uint8_t>>
    ipmiOemQGetProcInfo(ipmi::Context::ptr ctx, std::vector<uint8_t> reqData)
{
    qProcInfo_t* req = reinterpret_cast<qProcInfo_t*>(reqData.data());
    uint8_t numParam = sizeof(cpuInfoKey) / sizeof(uint8_t*);
    std::stringstream ss;
    ...
    if (oemData[procInfo][ss.str()].find(cpuInfoKey[req->paramSel]) ==
        oemData[procInfo][ss.str()].end())
        return ipmi::responseCommandNotAvailable();
    str = oemData[procInfo][ss.str()][cpuInfoKey[req->paramSel]];
...
ipmi::RspType<> ipmiOemQSetDimmInfo(ipmi::Context::ptr ctx,
                                    std::vector<uint8_t> reqData)
{
    qDimmInfo_t* req = reinterpret_cast<qDimmInfo_t*>(reqData.data());
    uint8_t numParam = sizeof(dimmInfoKey) / sizeof(uint8_t*);
    std::stringstream ss;
    std::string str;
    uint8_t len = reqData.size();

    std::string dimmType = dimmMap[req->dimmIndex];
    auto hostId = ipmi::boot::findHost(ctx->hostIdx);
    if (!hostId)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
    ...
    oemData[dimmInfo][ss.str()][KEY_DIMM_INDEX] = req->dimmIndex;
    oemData[dimmInfo][ss.str()][KEY_DIMM_TYPE] = dimmType;
    str = bytesToStr(reqData.data() + 5, len);
    oemData[dimmInfo][ss.str()][dimmInfoKey[req->paramSel]] = str.c_str();
    flushOemData();
    return ipmi::responseSuccess();
}
    ...
    ipmiOemQGetDimmInfo(ipmi::Context::ptr ctx, std::vector<uint8_t> reqData)
{
    ...
    if (oemData[dimmInfo].find(ss.str()) == oemData[dimmInfo].end())
        return ipmi::responseCommandNotAvailable();
    if (oemData[dimmInfo][ss.str()].find(dimmInfoKey[req->paramSel]) ==
        oemData[dimmInfo][ss.str()].end())
        return ipmi::responseCommandNotAvailable();
    str = oemData[dimmInfo][ss.str()][dimmInfoKey[req->paramSel]];
    int data_length = strToBytes(str, res);
    std::vector<uint8_t> response(&res[0], &res[data_length]);
    return ipmi::responseSuccess(response);
}
