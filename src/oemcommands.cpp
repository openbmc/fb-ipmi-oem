    ...
    [[maybe_unused]] ipmi::Context::ptr ctx, std::vector<uint8_t> reqData)
{
    ...
    {
        case BankType::mca:
            if (pHdr->bankHdr.version >= 5 ||
                bData.size() >= sizeof(CrdMcaBankV4))
                res = handleMcaBank<CrdMcaBankV4>(*pHdr, bData, dumpState, ss);
            else if (bData.size() >= sizeof(CrdMcaBankV3))
        ...
        case BankType::cpuWdt:
            if (pHdr->bankHdr.version >= 5)
                res = handleCpuWdtBank<CrdCpuWdtBankV5>(bData, dumpState, ss);
            else if (pHdr->bankHdr.version >= 4 ||
                     (bData.size() >= sizeof(CrdCpuWdtBankV4) &&
                      bData.size() < sizeof(CrdCpuWdtBank)))
