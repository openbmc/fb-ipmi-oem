                                   ...
                                   std::vector<uint8_t> req)
{
    ...
    std::string sysConfig = KEY_SYS_CONFIG + std::to_string(*hostId);

    std::stringstream ss;
    ss << std::hex;
    ss << std::setw(2) << std::setfill('0') << (int)index;

    oemData[sysConfig][ss.str()][KEY_DIMM_INDEX] = index;
    oemData[sysConfig][ss.str()][KEY_DIMM_TYPE] = type;
    oemData[sysConfig][ss.str()][KEY_DIMM_SPEED] = speed;
    oemData[sysConfig][ss.str()][KEY_DIMM_SIZE] = size;
