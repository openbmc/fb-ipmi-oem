...
std::string getMotherBoardFruPath()
{
    ...
    size_t hostPosition;
    std::function<bool(const std::string&)> candidate =
    ...
    if (platform)
    {
        ...
        if (hostPosition > 0)
        {
            std::string matcher = "_" + std::to_string(hostPosition);
            candidate = [needle = matcher](const std::string& p) {
...
int8_t getFruData(std::string& data, std::string& name)
{
    ...
    //If getMotherBoardFruPath() not return "", prefer a direct Asset read on that object
    if (!baseBoard.empty())
    {
        try
        {
            ...
            std::string service = getService(
                *dbus, "xyz.openbmc_project.Inventory.Decorator.Asset", baseBoard);
