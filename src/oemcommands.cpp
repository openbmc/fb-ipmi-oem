...
int8_t sysConfig(std::vector<std::string>& data, size_t pos)
{
    ...
    int ii = 0;
    while (ii != sysObj[dimmInfo][ii][KEY_DIMM_INDEX])
    {
        std::string speedSize = sysObj[dimmInfo][ii][DIMM_SPEED];
        ...
        std::string type = sysObj[dimmInfo][ii][DIMM_TYPE];
        std::string dualInlineMem = sysObj[dimmInfo][ii][KEY_DIMM_TYPE];
        strToBytes(type, res);
