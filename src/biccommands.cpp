...
ipmi::RspType<std::vector<uint8_t>>
    ipmiOemBicHandler(std::vector<uint8_t> inputReq)
{
    ...
    {
        std::copy(&inputReq.at(DATA_BYTE_IDX),
                  &inputReq.at(DATA_BYTE_IDX) + (dataLen - DATA_BYTE_IDX),
                  back_inserter(data));
    }
    ...
    std::copy(res->payload.raw.data(),
              res->payload.raw.data() + res->payload.size(),
              back_inserter(outputRes));
